...
class SearchableDropdown extends StatefulWidget {
  ...
  String? label;
  dynamic Model;
  final RxList<TextEditingController> controller;
  int index = 0;
  SearchableDropdown(this.endPoint, this.Model, this.label,this.controller,this.index);
  @override
        ...
        : Padding(
            ...
            child: TypeAheadField(
              textFieldConfiguration: TextFieldConfiguration(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: AnimatedSwitcher(
                    ...
                    child: widget.controller[widget.index].text.isEmpty
                        ? Icon(Icons.search,
                            color:
                                Colors.grey.shade500) // أيقونة بحث عند الفراغ
                        : IconButton(
                            icon:
                                Icon(Icons.close, color: Colors.grey.shade600),
                            onPressed: () {
                              widget.controller[widget.index].text = '';
                              newDocumentController.patientId[widget.index].value = '0';
                              print(newDocumentController.patientId[widget.index].value);
                              isSearched = false;
                            }, // زر مسح عند وجود نص
                          ),
                  ),
                ...
                if (suggestion['isMessage'] == true) {
                  return ListTile(
                    title: Text(
                      ...
                      style: TextStyle(
                        color:
                            suggestion['message'] == 'لم نعثر على نتائج لبحثك'
                                ? ColorConst.redColor
                  ...
                  if (suggestion['text'] == null && isSearched == true) {
                    ...
                    return ListTile(
                      ...
                      contentPadding: EdgeInsets.symmetric(
                          horizontal: 16), // تعديل المسافات
                      visualDensity:
                          VisualDensity.comfortable, // مسافات داخلية أكبر
                      shape: RoundedRectangleBorder(
                        // نفس الحواف المدورة
                        borderRadius: BorderRadius.circular(8),
                      ),
              ...
              onSuggestionSelected: (dynamic suggestion) {
                if (isSearched) {
                  widget.controller[widget.index].text = suggestion['text'];
                 String idType = suggestion['id'].toString();
                  print(idType);
                  print(widget.index);
                  newDocumentController.patientId[widget.index].value = idType;
                  print(newDocumentController.patientId[widget.index].value);
                } else {
                  widget.controller[widget.index].text = suggestion['name'];
                  String idType = suggestion['id'].toString();
                  print(idType);
                  print(widget.index);
                  newDocumentController.patientId[widget.index].value = idType;
